 ...
 * @brief Modbus master RTU driver interface for STM32G474RE.
 *        Differences from G070 driver:
 *          - GPIO clock via RCC->AHB2ENR (G474) instead of RCC->IOPENR (G070)
 *          - DMA clock via RCC->AHB1ENR instead of RCC->AHBENR
 *          - APB2ENR for USART1, APB1ENR1 for TIM7
 * @version 1.0.0
 * @date 2025-08-31
 *
...
static void master_usart_send(volatile const modbus_buf_t *tx_msg, modbus_buf_size_t msg_len)
{
    if ((tx_msg != NULL) && (msg_len > 0))
    {
        ...
        MODBUS_MASTER_DMA_chanel->CCR  |= DMA_CCR_TCIE | DMA_CCR_TEIE | DMA_CCR_DIR | DMA_CCR_MINC | DMA_CCR_PL;
...
static void USART_GPIO_init(void)
{
    // G474: GPIO clocks on AHB2
    RCC->MODBUS_MASTER_GPIO_CLK_REG |= MODBUS_MASTER_USART_GPIO_RX_TX_CLK_EN;
    RCC->MODBUS_MASTER_GPIO_CLK_REG |= MODBUS_MASTER_USART_GPIO_DE_CLK_EN;
...
static void USART_DMA_init(void)
{
    // G474: DMA clock on AHB1 (G070 used AHBENR)
    RCC->MODBUS_MASTER_DMA_CLK_REG |= MODBUS_MASTER_DMA_CLK_EN;

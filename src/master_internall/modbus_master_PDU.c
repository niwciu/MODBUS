...
static void set_inreg_hreg_value_from_modbus_msg(const modbus_buf_t *msg_data_ptr, modbus_adr_t start_adr, modbus_data_qty_t reg_qty, modbus_reg_t *rw_data_ptr);
...
static modbus_ret_t set_coil_din_value_from_modbus_msg(const modbus_buf_t *data_state_ptr, modbus_data_qty_t coil_din_qty, modbus_coil_disin_t *data_tab);
...
modbus_ret_t modbus_master_write_single_reg_req(modbus_msg_t *modbus_msg, modbus_adr_t adr, void *rw_data_ptr)
{
    if (RET_OK == check_null_ptr_correctness(modbus_msg))
    {
        return write_single_reg_coil_request(&modbus_msg->req, MODBUS_WRITE_SINGLE_REGISTER_FUNC_CODE, adr, rw_data_ptr);
    }
    else
    {
        return RET_NULL_PTR_ERROR;
...
modbus_ret_t modbus_master_write_single_coil_req(modbus_msg_t *modbus_msg, modbus_adr_t adr, void *rw_data_ptr)
{
    if (RET_OK == check_null_ptr_correctness(modbus_msg))
    {
        return write_single_reg_coil_request(&modbus_msg->req, MODBUS_WRITE_SINGLE_COIL_FUNC_CODE, adr, rw_data_ptr);
    }
    else
    {
        return RET_NULL_PTR_ERROR;
...
modbus_ret_t modbus_master_write_multiple_reg_req(modbus_msg_t *modbus_msg, modbus_adr_t adr, modbus_data_qty_t reg_qty, void *rw_data_ptr)
{
    if (RET_OK == check_null_ptr_correctness(modbus_msg))
    {
        if ((reg_qty <= MODBUS_MAX_WRITE_REG_QTY) && (reg_qty >= MODBUS_MIN_REG_COIL_QTY))
        {
            ...
            {
                uint16_t data = *((modbus_reg_t *)(rw_data_ptr) + i);
...
modbus_ret_t modbus_master_write_multiple_coils_req(modbus_msg_t *modbus_msg, modbus_adr_t adr, modbus_data_qty_t coils_qty, void *rw_data_ptr)
{
    if (RET_OK == check_null_ptr_correctness(modbus_msg))
    {
        ...
        if ((coils_qty <= MODBUS_MAX_WRITE_COILS_QTY) && (coils_qty >= MODBUS_MIN_REG_COIL_QTY))
        {
            ...
            {
                modbus_coil_disin_t data = *((modbus_coil_disin_t *)(rw_data_ptr) + i);
...
static void set_inreg_hreg_value_from_modbus_msg(const modbus_buf_t *msg_data_ptr, modbus_adr_t start_adr, modbus_data_qty_t reg_qty, modbus_reg_t *rw_data_ptr)
{
    for (modbus_data_qty_t i = 0; i < reg_qty; i++)
    {
        ...
        set_register_value((modbus_reg_t **)(rw_data_ptr), start_adr + i, reg_data);
    }
}
static modbus_ret_t update_master_data_from_modbus_msg(const modbus_req_resp_t *resp, const modbus_req_resp_t *req, modbus_fun_code_t fun_code, void *rw_data_ptr)
{
...
static void update_master_specific_data_type_from_modbus_msg(const modbus_buf_t *resp, const modbus_buf_t *req, modbus_fun_code_t fun_code, void *rw_data_ptr)
{
    ...
    case MODBUS_READ_HOLDING_REGISTERS_FUNC_CODE:
        // set_inreg_hreg_value_from_modbus_msg(&resp[MODBUS_RESP_READ_DATA_IDX], 0, data_qty, (modbus_reg_t *)(rw_data_ptr));
        break;
    default:
...
static modbus_ret_t set_coil_din_value_from_modbus_msg(const modbus_buf_t *data_state_ptr , modbus_data_qty_t coil_din_qty, modbus_coil_disin_t *data_tab)
{
    ...
    for (modbus_data_qty_t i = 0; i < coil_din_qty; i++)
    {
        ...
        {
            status = set_coil_state((modbus_coil_disin_t**)(data_tab), i, 1);
            // if (NULL != data_tab)
            // {
            //     *(data_tab+i) = 1;
            //     status = RET_OK;
        ...
        {
            // status = set_coil_state((modbus_coil_disin_t**)(data_tab) ,i, 0);
            if (NULL != data_tab)
            {
                *(data_tab+i) = 0;
                status=  RET_OK;

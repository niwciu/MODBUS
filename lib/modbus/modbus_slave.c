...
static e_modbus_init_t handle_modbus_slave_init(modbus_mode_t mode, baud_t baud_rate, parity_t parity, modbus_device_ID_t slave_ID);
static bool slave_ID_is_correct(modbus_device_ID_t slave_ID);
...
static e_modbus_status_t init_modbus_driver(modbus_mode_t mode, baud_t baud_rate, parity_t parity);
 ...
 */
e_modbus_init_t modbus_slave_init(modbus_mode_t mode, baud_t baud_rate, parity_t parity, modbus_device_ID_t slave_ID)
{
    e_modbus_init_t init_status = MODBUS_INIT_INVALID_ARG_ERR;
    slave_RTU_driver = NULL;
    if (validate_init_arguments(mode, baud_rate, parity, slave_ID))
    {
        init_status = handle_modbus_slave_init(mode, baud_rate, parity, slave_ID);
    }
    return init_status;
}
...
static e_modbus_init_t handle_modbus_slave_init(modbus_mode_t mode, baud_t baud_rate, parity_t parity, modbus_device_ID_t slave_ID)
{
    e_modbus_init_t init_status = MODBUS_INIT_DRIVER_INIT_ERR;
    init_modbus_data_buffers_and_queues(mode);
    init_modbus_slave_internall_data(slave_ID);
    if (init_modbus_driver(mode, baud_rate, parity) == MODBUS_OK)
    {
        init_status = MODBUS_INIT_OK;
    }
    return init_status;
}
...
static e_modbus_status_t init_modbus_driver(modbus_mode_t mode, baud_t baud_rate, parity_t parity)
{
    e_modbus_status_t ret = MODBUS_NOK;
    if (RTU == mode)
    {

...
PRIVATE volatile modbus_status_flag_t MODBUS_MASTER_FRAME_ERROR_FLAG = MODBUS_FLAG_UNKNOWN;
PRIVATE volatile modbus_status_flag_t MODBUS_MASTER_REQ_TRANSMITION_FLAG = MODBUS_FLAG_UNKNOWN;
PRIVATE volatile modbus_status_flag_t MODBUS_MASTER_RTU_CRC_ERROR_FLAG = MODBUS_FLAG_UNKNOWN;
...
static void init_msg_req_resp_data_buffers(void);
...
static e_modbus_init_t init_master_RTU_driver(baud_t baud_rate, parity_t parity);
static void init_master_internall_data(void);
 ...
 */
e_modbus_init_t modbus_master_init(modbus_mode_t mode, baud_t baud_rate, parity_t parity)
{
    e_modbus_init_t master_init_status = MODBUS_INIT_INVALID_ARG_ERR;
    init_msg_req_resp_data_buffers();
    if (init_arguments_are_valid(mode, baud_rate, parity))
    {
        ...
        master_init_status = init_master_RTU_driver(baud_rate, parity);
        init_master_internall_data();
    }
    return master_init_status;
}
 ...
 *
 * @note Assumes `master_RTU_driver` is properly initialized and `msg_buf` contains the request to be sent.
 */
static void modbus_master_send_req_from_msg_buf(void)
{
    MODBUS_MASTER_TIMER_3_5_CHAR_FLAG = MODBUS_FLAG_CLEARED;
    master_RTU_driver->send(msg_buf->req.data, msg_buf->req.len);
    master_RTU_driver->enable_rcev(&msg_buf->resp);
...
static e_modbus_init_t init_master_RTU_driver(baud_t baud_rate, parity_t parity)
{
    ...
    if (RTU_driver_is_valid(master_RTU_driver))
    {
        ...
        master_RTU_driver->subscribe_t_1_5_char_expired_cb(modbus_master_T_1_5_char_expired_callback);
        master_RTU_driver->subscribe_t_3_5_char_expired_cb(modbus_master_T_3_5_char_expired_callback);
        master_RTU_driver->subscribe_modbus_frame_error_cb(modbus_master_frame_error_callback);
        status = MODBUS_INIT_OK;
    }
    return status;
}
...
static void init_master_internall_data(void)
{
    // setting up all flags ans modus_master_stateMachine
...
static void modbus_master_resp_recived_state_handling(void)
{
    ...
    {
        // do nothing untill 3_5TFlag or timeout event accour

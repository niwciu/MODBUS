...
extern struct modbus_RTU_driver_struct *master_RTU_driver;
...
{
    modbus_master_init(UNKNOWN_MODE, valid_baud, valid_parity);
...
{
    modbus_master_init(RTU, valid_baud, valid_parity);
    for (int i = 0; i < MODBUS_MASTER_MAX_MSG_QUEUE_ITEMS; i++)
    {
...
{
    modbus_master_init(RTU, valid_baud, valid_parity);
    TEST_ASSERT_NOT_NULL(master_RTU_driver);
}

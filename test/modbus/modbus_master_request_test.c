...
TEST(Modbus_Master_Requests, WriteSingleRegister)
{
    ...
    modbus_ret_t status= modbus_master_write_single_reg_req(PDU_frame, adr, value);
...
TEST(Modbus_Master_Requests, WriteSingleCoilOn)
{
    ...
    modbus_ret_t status= modbus_master_write_single_coil_req(PDU_frame, adr, coil_state);
...
TEST(Modbus_Master_Requests, WriteMultipleRegisters)
{
    ...
    modbus_ret_t status = modbus_master_write_multiple_reg_req(PDU_frame, adr, reg_qty, values);
...
TEST(Modbus_Master_Requests, WriteMaxQtyMultipleRegisters)
{
    ...
    status = modbus_master_write_multiple_reg_req(PDU_frame, adr, reg_qty, values);
    TEST_ASSERT_EQUAL_INT16(MODBUS_WRITE_MULTI_REQUEST_LEN+(reg_qty*2), status);
}
...
TEST(Modbus_Master_Requests, Write5MultipleCoils)
{
    ...
    status=modbus_master_write_multiple_coils_req(PDU_frame, adr,coils_qty, coil_states);

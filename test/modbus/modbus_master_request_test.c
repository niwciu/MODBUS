...
TEST(Modbus_Master_Requests, ReadSingleHoldingRegisterRequest)
{
    ...
    modbus_reg_t status = modbus_master_read_holding_reg_req(PDU_frame, adr, len);
...
TEST(Modbus_Master_Requests, ReadSingleInputRegisterRequest)
{
    ...
    modbus_reg_t status = modbus_master_read_input_reg_req(PDU_frame, adr, len);
...
TEST(Modbus_Master_Requests, ReadMaxQtyInputRegisterRequest)
{
    modbus_adr_t adr = 0x0005;
    modbus_data_qty_t len = MODBUS_MAX_READ_REG_QTY;
    modbus_ret_t status = modbus_master_read_input_reg_req(PDU_frame, adr, len);
...
TEST(Modbus_Master_Requests, ReadDiscreteInputsRequest)
{
    ...
    modbus_ret_t status = modbus_master_read_discrete_inputs_req(PDU_frame, adr, input_qty);
...
TEST(Modbus_Master_Requests, ReadCoilsRequest)
{
    ...
    modbus_ret_t status = modbus_master_read_coils_req(PDU_frame, adr, input_qty);
...
TEST(Modbus_Master_Requests, WriteSingleRegister)
{
    ...
    status= modbus_master_write_single_reg_req(PDU_frame, adr);
...
TEST(Modbus_Master_Requests, WriteSingleCoilOn)
{
    ...
    status= modbus_master_write_single_coil_req(PDU_frame, adr);

    TEST_ASSERT_EQUAL_UINT8(MODBUS_WRITE_SINGLE_COIL_FUNC_CODE, PDU_frame[MODBUS_FUNCTION_CODE_IDX]);
    TEST_ASSERT_EQUAL_UINT16(adr, read_u16_from_buf(PDU_frame + MODBUS_REQUEST_ADR_IDX));
    TEST_ASSERT_EQUAL_UINT16(COIL_ON, read_u16_from_buf(PDU_frame + MODBUS_REQUEST_LEN_IDX));
    TEST_ASSERT_EQUAL_INT16(MODBUS_WRITE_SINGLE_REQUEST_LEN, status);
}
...
TEST(Modbus_Master_Requests, WriteMultipleRegisters)
{
    ...
    modbus_ret_t status = modbus_master_write_multiple_reg_req(PDU_frame, adr, reg_qty);
    ...
    TEST_ASSERT_EQUAL_HEX16(0x5A5A, read_u16_from_buf(PDU_frame + MODBUS_REQUEST_WRITE_MULTI_DATA_IDX));
    TEST_ASSERT_EQUAL_HEX16(0xA5A5, read_u16_from_buf(PDU_frame + MODBUS_REQUEST_WRITE_MULTI_DATA_IDX + 2));
    TEST_ASSERT_EQUAL_HEX16(0x5A5A, read_u16_from_buf(PDU_frame + MODBUS_REQUEST_WRITE_MULTI_DATA_IDX + 4));
    TEST_ASSERT_EQUAL_HEX16(0xA5A5, read_u16_from_buf(PDU_frame + MODBUS_REQUEST_WRITE_MULTI_DATA_IDX + 6));
    TEST_ASSERT_EQUAL_HEX16(0x5A5A, read_u16_from_buf(PDU_frame + MODBUS_REQUEST_WRITE_MULTI_DATA_IDX + 8));
...
TEST(Modbus_Master_Requests, WriteMaxQtyMultipleRegisters)
{
    ...
    status = modbus_master_write_multiple_reg_req(PDU_frame, adr, reg_qty);
    TEST_ASSERT_EQUAL_INT16(MODBUS_WRITE_MULTI_REQUEST_LEN+(reg_qty*2), status);
}
...
TEST(Modbus_Master_Requests, Write5MultipleCoils)
{
    ...
    TEST_ASSERT_EQUAL_HEX8_ARRAY(expected_coil_states_in_PDU,PDU_frame+MODBUS_REQUEST_WRITE_MULTI_DATA_IDX,expected_byte_cnt);
